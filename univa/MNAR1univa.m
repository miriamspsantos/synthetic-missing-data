function [outputX, idx_xs] = MNAR1univa(inputX, mr, ftypes, Y, flag)
% MNAR1univa(inputX, mr, ftypes, Y) performs IMuniva (Twala09)
...
% variable (as is done in the paper). For the case of IMuniva, xs is also
% considered the determining feature, and its lowest values are removed.
%
%
% INPUT:
...
% flag = 0;
% flag = 1;
% outputX = MNAR1univa(X, mr, ftypes, Y, flag);
% [outputX, idx_xs] = MNAR1univa(X, mr, ftypes, Y, flag)
%
%
% REFERENCES:
% @article{Twala09,
...
% Determine the N lowest values of the simulated and determining feature xs
[~, xs_order] = sort(xs);
