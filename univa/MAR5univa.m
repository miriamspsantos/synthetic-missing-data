function [outputX, idx_xs, idx_xd] = MAR5univa(inputX, mr, varargin)
%  MAR5univa(inputX, mr, varargin) performs MAR4 (Rieger10)
%
% INPUT:
...
% EXAMPLE:
% X = rand(5,10)
% [outputX, pb, idx_xs, idx_xd] = MAR5univa(X, 50, 8) % 8 features missing
% [outputX, pb, idx_xs, idx_xd] = MAR5univa(X, 50,1,3) % in 1, based
% on 3
% [outputX, pb, idx_xs, idx_xd] = MAR5univa(X, 50) % randomly in one
%
%
%
% REFERENCES:
...
if mod(N,2) ==0
    ...
    Nlow = floor(N/2)+1; % Nlow has priority (gets 1 more value than Nhigh)
    Nhigh = floor(N/2);
end

% Determine the N/2 lowest values of the determining feature xd
lowestNindex = xd_order(1:Nlow);

% Determine the N/2 highest values of the determining feature xd
highestNindex = xd_order(end:-1:end-(Nhigh-1));

% Introduce the missing values in the features nxs corresponding to the
% indices of X where xd has the lowest and highest values
