function outputX = MNAR2unifo(inputX, mr, ftypes, Y)
% MNAR2unifo(inputX, mr) performs IMunifo (Twala09)
...
% target class Y is chosen to insert missing values (Ay). In the case of
% IMunifo, the Ay feature itself is used as determining feature.
%
%
% For k% of missing values in the whole dataset, 2k% of missing values are
% simulated in Ay. For each Ay, its 2k quantil (cutoff = percentil 2k%) is
...
% mr = 10;
% outputX = MNAR2unifo(X, mr, ftypes, Y);
%
%
% X = rand(100,5);
% ftypes = [0 0 0 0 0 0];
...
for i = 1:length(varPairs)
    ...
    % Check where which vars are to be missing
    ...
    for c = 1:size(X_xs,2)
        % Determine the N lowest values of the determining feature xs
        [~, xs_order] = sort(X_xs(:,c));
        lowestNindex = xs_order(1:N);
        outputX(lowestNindex, Pairs.(varPairs{i})(idx_xs(c))) = NaN;
    end
